...
## PM review v17 - final production closure (2026-08-25)

Đây là review tổng hợp cuối để dev đóng sản phẩm một lượt. PM đã đọc working
tree hiện tại, chạy test/build/audit trên code thật, migrate một Postgres rỗng,
gọi runtime API và chạy lại embedded golden path. Những vòng v16 trở xuống là
lịch sử; không dùng comment dev hoặc số test đơn lẻ để kết luận release.

### Release decision

| Phạm vi | Quyết định | Lý do |
|---|---|---|
| Dev/internal demo trên máy hiện tại | **GO** | core embedded chạy full/incremental, API và FE build khỏe |
| Internal UAT có người vận hành | **GO có điều kiện** | được phép dùng để đóng UX và runbook, không dùng dữ liệu production |
| External pilot/production | **NO-GO** | outbox chưa đúng semantics Airbyte thật; topology/supply chain/DR/UAT chưa có evidence release |
| Push/tag release hiện tại | **NO-GO** | 46 tracked file thay đổi, nhiều untracked file, không có remote; chưa có commit tái tạo được |

### Bằng chứng PM đã chạy độc lập

| Kiểm tra | Kết quả và giới hạn |
|---|---|
| Backend suite | `304 passed, 32 skipped`; mặc định skip engine contract và 20 bài Postgres live |
| Postgres live | `20 passed` cho production core, forced password và outbox fault suite |
| Migration sạch | DB rỗng lên `b4f8c21d7e93 (head)`; `alembic check` không có operation mới |
| Schema drift | current/head khớp, tất cả model-declared index hiện diện |
| Runtime API/RBAC | `scripts/audit-api.py` exit `0`; một info `422` trước permission check, không phải bypass |
| Secret scanner | không thấy plaintext credential khi chạy bằng đúng runtime DB URL; command mặc định từ host không tự bind secret installer |
| Embedded E2E | 2.500 row full refresh; incremental đọc đúng 2 row mới; destination khớp |
| Cancel | request được nhận nhưng job kết thúc `SUCCEEDED`; script đúng khi không tính là proof |
| Builder | skip do không có Builder project; chưa có proof vòng này |
| FE | lint/typecheck/build pass; i18n `807/807`; `/change-password` có trong production bundle |
| Dependency | `npm audit --omit=dev` và `pip-audit` sạch tại thời điểm review |
| Kustomize | product và connector-policy overlays render được; `kubeconform` không có trên máy PM |
| Browser UAT | browser tích hợp không khả dụng; chưa có desktop/mobile/accessibility evidence mới |

### P0-CORE-001 - outbox chưa bảo vệ crash window với Airbyte thật

`actors.create` mở ledger, gọi engine, rồi mới lưu ref tại
`backend/app/services/actors.py:357-402`. Tuy nhiên Airbyte adapter gửi
`workspaceId`, definition, config và name tại
`backend/app/adapters/airbyte_api/adapter.py:728-746` và
`backend/app/adapters/airbyte_api/adapter.py:782-799`; Product UUID không trở
thành Airbyte resource ID. Airbyte tự trả một ID mới.

Nếu process chết sau Airbyte HTTP `200` nhưng trước `outbox.engine_created()`,
ledger còn `PENDING` và không có `engine_ref`. Sweeper tại
`backend/app/services/outbox.py:247-253` thử xóa bằng Product UUID. Đây không
phải `sourceId`/`destinationId` của Airbyte, vì vậy resource chứa credential có
thể bị orphan.

Test được xem là bằng chứng cho tình huống này lại cho fake engine tự tạo ref từ
`request.product_resource_id` và tìm resource theo suffix tại
`backend/tests/test_outbox_faults.py:45-64`. Đó là khả năng embedded/fake có nhưng
Airbyte API không cung cấp. Vì vậy `20 passed` không đóng được rủi ro chính.

Phạm vi cũng chưa đúng với claim của model: hiện outbox chỉ được gọi ở create
source/destination. Actor update/delete tại
`backend/app/services/actors.py:470-569` và pipeline create/update/delete tại
`backend/app/services/pipelines.py:350-550` vẫn mutation engine không có cùng
ledger. Unique `(product_resource_id, operation)` tại
`backend/app/models/outbox.py:119-122` còn không lưu được lịch sử nhiều lần
UPDATE của một resource. Worker chỉ log `outbox.stuck` tại
`backend/app/worker.py:224-242`; `/metrics` và `alerts.yaml` chưa có metric/rule
tương ứng.

**Acceptance bắt buộc:**

1. Chọn semantics idempotency/compensation dùng được với Airbyte thật. Không dựa
   vào việc delete bằng Product UUID nếu API không hỗ trợ.
2. Kill process thật giữa engine HTTP success và lưu ref trên Airbyte 1.8.5
   auth-enabled; sau sweeper không còn orphan và Product DB/engine hội tụ.
3. Bọc create/update/delete của source, destination, pipeline hoặc thu hẹp rõ
   contract và có cơ chế bù tương đương cho phần còn lại.
4. Mỗi mutation có event/operation ID riêng; sweeper an toàn khi có nhiều worker.
5. Export metric backlog/age/failure và alert thực tế có runbook, có bài test alert
   delivery.

### P0-REL-001 - chưa có một release có thể tái tạo

HEAD đang là `f6ce6ec`, nhưng toàn bộ implementation vòng mới nằm trong working
tree lớn chưa commit; repository không có remote. Test xanh chỉ chứng minh máy
hiện tại ở thời điểm này. Trước mọi deployment phải tạo một commit sạch và giữ
mapping bất biến `commit -> build ID -> image digest -> evidence`.

Lane `.github/workflows/ci.yml:297-498` là scheduled/manual và chưa có remote run
xanh. Nó gọi `scripts/airbyte-application.py` tại
`.github/workflows/ci.yml:371-386`. Script này thử nhiều request shape để setup,
login và tạo Application; tests tại `backend/tests/test_ci_lane.py:380-410`
mock các shape dự đoán. Unit test không chứng minh flow operator/auth của chart
V2 hoạt động trên runner mới.

**Acceptance bắt buộc:** clean commit trên protected remote branch; CI từ chính
commit đó phải dựng topology mới, tạo Application bằng một procedure đã đo live,
chạy golden path và publish evidence/artifact gắn với digest. Không dùng file
evidence cũ v1 hoặc evidence có `build: unknown`.

### P0-PLAT-001 - chưa đạt yêu cầu “một file chạy toàn production”

`scripts/production.py:951-1028` render và apply AppBI manifests cùng connector
policy vào một cluster đã tồn tại. Nó không dựng Airbyte Helm release, hai
external Postgres, Redis/object storage, internal registry hay Application
credential. Trường `airbyte.chart_version` trong config mới là metadata; installer
không consume nó để cài engine.

`cmd_upgrade` gọi `scripts/backup.py dump` tại
`scripts/production.py:1388-1408` nhưng không truyền production config/secret
refs; backup script mặc định tìm local/docker AppBI DB và không tạo một paired
backup gồm Airbyte DB + object storage. `cmd_rollback` tại
`scripts/production.py:1777-1811` chỉ in hướng dẫn nhưng có thể tạo cảm giác lệnh
đã hoàn thành.

**Acceptance bắt buộc:** có đúng một entrypoint top-level cho production. Nếu hạ
tầng do IaC bên ngoài quản lý, entrypoint phải fail-fast và verify tất cả
prerequisite, sau đó deploy/migrate/issue credential/verify golden path. Nếu repo
quản lý hạ tầng, entrypoint phải orchestration IaC + Helm tương ứng. Backup,
upgrade, rollback và doctor đều consume cùng một config/secret contract; rollback
phải thực thi hoặc exit non-zero với trạng thái `NOT_IMPLEMENTED`.

### P0-SUPPLY-001 - chưa độc lập với GitHub/public registry

Rủi ro Airbyte ngừng cho pull hoặc upstream thay đổi chưa được đóng:

- Production overlay vẫn dùng tag `1.0.0` tại
  `deploy/kubernetes/overlays/production/kustomization.yaml:32-35`.
- Renderer tạo `newTag` tại `scripts/production.py:612-615`, không pin digest.
- CI vẫn `helm repo add` từ public Airbyte tại `.github/workflows/ci.yml:356`.
- `scripts/mirror.py` xây danh sách image bằng quy ước tên/tag; chưa render Helm
  chart và trích toàn bộ image/dependency thực tế.
- `mirror-lock.json` chưa được installer/CI/release gate consume như input bắt
  buộc; hiện cũng chưa có lock, SBOM, signature hoặc provenance artifact.

**Acceptance bắt buộc:** mirror nội bộ exact Helm chart, source/chart archive cần
cho license, toàn bộ image sau `helm template`, connector images trong launch
scope và product images; pin digest; tạo SBOM, ký, provenance và verify tại
install/release gate. Chạy clean runner với GitHub, Helm repo, Docker Hub/GHCR và
Airbyte registry public bị chặn. Nếu upstream biến mất, release và restore vẫn
phải chạy từ artifact nội bộ đã phê duyệt.

### P0-SEC-001 - login lockout tạo targeted denial of service

`backend/app/api/v1/auth.py:53-98` khóa một account thật sau 5 password sai và
tăng lockout tới 4 giờ. Attacker không cần biết password; họ chỉ cần lặp login
với email operator/admin để giữ tài khoản bị khóa. Trả lỗi generic giúp chống
enumeration nhưng không chống DoS. Nginx hiện chưa có rate limit theo IP.

**Acceptance bắt buộc:** rate limit tại ingress/WAF theo IP và theo deployment,
account policy không cho người ngoài kéo dài lockout vô hạn, đường khôi phục
admin được audit và MFA/SSO decision cho production. Giữ response generic; test
distributed attempts và recovery, không chỉ test một process.

### P0-EVIDENCE-001 - exact production topology chưa được chứng minh

Repo chưa có `deploy/production.yaml` thật, certification artifact cuối,
`mirror-lock`, SBOM/provenance hay remote CI artifact. `compatibility.yaml` vẫn có
`LIC-001: NOT_CLEARED` và `UAT-001..015: NOT_PROVEN`; năm vai trò tại
`docs/RUNBOOK-oncall.md:12-16` còn `TO BE ASSIGNED`.

E2E PM vừa chạy là embedded: full/incremental pass, nhưng cancel không tạo trạng
thái `CANCELLED`, Builder bị skip và evidence không định danh build. Nó không
thay thế auth-enabled Kubernetes evidence.

**Acceptance bắt buộc trên đúng commit/digest/topology production:**

1. Source/destination check, discover, full refresh, incremental cursor/dedup và
   row reconciliation.
2. Một job đủ dài để cancel thật tới terminal `CANCELLED`, cùng log/error mapping.
3. Builder test -> publish -> source -> sync nếu Builder nằm trong launch scope;
   nếu không, loại khỏi scope và UI.
4. Worker restart, engine restart, connector image cold pull và crash window của
   outbox.
5. NetworkPolicy/egress dưới CNI production, bao gồm allow-control và DNS.
6. Paired backup/restore AppBI DB + Airbyte DB + object storage + KEK trên một
   deployment khác; credential decrypt và reconcile được.
7. Upgrade và rollback drill; alert được gửi tới pager thật và có người ack.
8. Load/soak theo SLO, capacity/concurrency limit, retention/privacy deletion và
   failure-recovery report.

### P0-UAT-001 - FE build pass chưa đồng nghĩa sản phẩm dùng được

Forced password flow đã có route và build được. Wizard đã lọc connector theo
server-side launch scope và có progressive disclosure. Tuy nhiên checklist chính
vẫn đi tới `/sources/new` thay vì mở journey liên tục tại
`frontend/src/app/(main)/overview/page.tsx:243-249`; người dùng phải tự hiểu bước
tiếp theo. `frontend/src/app/change-password/page.tsx:127` dùng class
`text-status-danger` trong khi token hiện có là `text-danger`. Đây là polish,
không phải lý do trì hoãn core, nhưng phải được bắt trong một UAT cuối.

Browser không khả dụng trong phiên PM nên chưa có bằng chứng viewport,
accessibility, focus/keyboard, empty/error/loading states hoặc complete first-user
journey. Các script UI tồn tại nhưng chưa có artifact từ release commit.

**Acceptance bắt buộc:** một người không tham gia phát triển phải hoàn thành
bootstrap/login/change password -> source -> destination -> pipeline -> first run
-> xem log/cancel/xử lý lỗi trên desktop 1366px và mobile. Ghi video/screenshot,
thời gian hoàn thành, điểm kẹt và checklist accessibility cơ bản. UI chỉ hiện
connector đã chứng nhận.

### P1 gộp vào cùng release, không mở vòng review riêng

- `scripts/scan-plaintext-secrets.py` cần nhận `--config` hoặc được gọi qua
  `production.py`, vì command mặc định không đọc được installer-generated DB
  secret từ host. Audit remediation phải chạy lặp lại được kể cả sau lần `--fix`.
- Run list còn query theo từng unique pipeline tại
  `backend/app/api/v1/runs.py:57-89`; thêm query-count regression và đo trong load
  test thay vì tối ưu cảm tính.
- Sửa token màu change-password, nối onboarding vào `?journey=1`, và bảo đảm CTA
  tôn trọng permission. Đóng cả ba trong UAT polish, không gửi PM từng mục.

### Phần đã được chấp nhận và không mở lại vô lý

- AppBI dùng adapter gọi Airbyte, không copy backend Airbyte vào product.
- Product DB và Airbyte DB tách biệt; engine mapping là boundary đúng.
- Airbyte chart/app version đã pin theo compatibility direction; config fail
  closed với engine không được chứng nhận.
- Readiness shallow/deep, migrations, schema drift và DB separation guard.
- Forced password backend, session version revoke, RBAC và connector launch-scope
  enforcement.
- Recursive secret split cho dữ liệu mới; runtime scan bằng đúng DB URL không
  thấy plaintext credential.
- FE lint/typecheck/i18n/build; npm/pip audit sạch tại commit review.
- Embedded full/incremental golden path và warehouse row reconciliation.

### Phạm vi release đầu để lên production sớm

Chỉ công bố **PostgreSQL source -> PostgreSQL destination**, concurrency thấp,
business-hours pilot và có operator theo dõi. Không cam kết “mọi connector
Airbyte”. Mỗi connector mở thêm phải có evidence gắn với exact engine version,
connector image digest, commit và golden dataset. Cách này đưa sản phẩm lên sớm
mà không biến catalog 654 connector thành lời hứa chưa kiểm chứng.

### Gói duy nhất dev phải trả lại PM

1. Clean commit SHA trên remote và CI URL xanh.
2. Product/engine/chart/connector image digests, `mirror-lock`, SBOM, signature và
   provenance.
3. Production config đã redact cùng một lệnh install/doctor/status/rollback.
4. Evidence-v2 cho toàn bộ exact-topology golden path và failure drills.
5. DR restore, alert delivery, load/soak và UAT artifacts.
6. `LIC-001`, on-call rota và `UAT-001..015` đều có owner/evidence/status hợp lệ.
7. `release-gate.py record/check` PASS và artifact định danh đúng commit/digest.

Dev không báo từng lỗi đã sửa. Chỉ gửi lại release candidate khi bảy đầu ra trên
cùng trỏ tới một commit và một bộ digest. Khi đó PM chạy clean-room rehearsal lần
cuối; không phát sinh kiến trúc mới ngoài regression do chính RC tạo ra.

---

## PM review v16 - core backend và frontend usability (2026-08-25)

Review này đọc code đang chạy, diff chưa commit của dev, gọi live API với bốn
vai trò, chạy Postgres thật và kiểm trực tiếp schema/response. Kết luận không dựa
vào số test hoặc comment dev. Do browser tích hợp không khả dụng trong phiên
review, phần FE được xác nhận bằng source, API payload và production build; chưa
có screenshot/mobile interaction mới và đó vẫn là một acceptance gap.

### Quyết định

- **Backend demo: GO cho dev/internal demo trên máy hiện tại.**
- **Production hoặc external pilot: NO-GO.** Còn ba lỗi core sản phẩm và một lỗi
  release-security phải đóng; đây không phải backlog polish.
- **FE: build được nhưng chưa UAT-ready.** Luồng production bootstrap hiện không
  thể hoàn tất và luồng tạo integration làm người dùng đi qua quá nhiều lựa chọn
  không dùng được.

### Bằng chứng độc lập đã chạy

| Kiểm tra | Kết quả |
|---|---|
| Runtime | 5 container Running; UI/API/deep readiness `200`; khoảng 382 MiB RAM tổng |
| Backend unit/integration mặc định | `290 passed, 18 skipped` |
| Core trên Postgres thật | `6 passed`: bootstrap, 20 concurrent trigger, active-run invariant, recovery taxonomy, session revoke, timeout |
| Schema | Alembic `f2c0a15b8e37 (head)`; `alembic check` và drift/index check sạch |
| Live auth/RBAC | anonymous `401`; admin/dataadmin/operator/analyst đúng matrix; analyst mutation `403` |
| Secret response | source/destination detail chỉ trả descriptor mask `********` trong đường bình thường |
| FE | lint pass, typecheck pass, i18n `794/794`, Next production build pass |
| Dependency audit | npm runtime `0 vulnerabilities`; pip `No known vulnerabilities` |

### P0-CORE-AUTH - production bootstrap dừng ở màn trắng

Backend khai báo `password_change_required` và comment rằng FE sẽ chuyển người
dùng sang màn đổi mật khẩu tại `backend/app/schemas/domain.py:53-66`. Enforcement
ở `backend/app/api/deps.py:57-77` chặn mọi dependency `CtxDep`. Tuy nhiên
`/auth/me` cũng nhận `CtxDep` tại `backend/app/api/v1/auth.py:89-91`, nên tài
khoản bootstrap không thể tự đọc lại session sau reload.

Phía FE không khai báo `password_change_required` trong `CurrentUser`, không có
client gọi `/auth/change-password`, không có route đổi mật khẩu, và login luôn
`router.replace('/overview')` tại `frontend/src/app/login/page.tsx:25-33`.
`AppShell` chỉ redirect lỗi `401`; lỗi `403 PASSWORD_CHANGE_REQUIRED` làm
`!user` rồi render `null` tại `frontend/src/components/layout/AppShell.tsx:55-75`.

Cùng lúc đó login page luôn prefill và hiển thị
`admin@appbi.local / Admin@12345` tại
`frontend/src/app/login/page.tsx:20-21,86-95`, kể cả production build. Backend
production không tạo tài khoản này, nên vừa gây nhầm lẫn vừa làm hỏng kỳ vọng
"không có default credential" ở bề mặt sản phẩm.

**Acceptance bắt buộc:**

1. Có route đổi mật khẩu nằm ngoài `AppShell`; login và session bootstrap đều
   chuyển đúng vào route đó.
2. `/auth/me` hoặc một endpoint session tối thiểu dùng `UserDep`, cho phép đọc
   flag trong khi mọi product route vẫn bị chặn.
3. FE gọi `/auth/change-password`, nhận cookie mới, reload được `/overview`; mọi
   session cũ bị `401 SESSION_REVOKED`.
4. Demo credential chỉ render khi một demo flag được bind rõ ràng; rendered
   production manifest phải assert flag false.
5. Member invite dùng cùng `password_problems()`, đặt
   `password_change_required=true`; hiện schema cho 8 ký tự và route hash trực
   tiếp tại `backend/app/api/v1/auth.py:256-270`, lệch policy 12 ký tự.
6. Browser test bắt đầu từ database production rỗng, bootstrap, đổi mật khẩu,
   logout/login lại và reload trang.

### P0-CORE-SECRET - nested secret có thể lưu plaintext

`split_configuration()` tuy ghi là recursive nhưng chỉ xét direct property và
một tầng `oneOf` tại `backend/app/services/catalog.py:286-325`. Đã gọi chính hàm
này với spec đang bundle của `destination-bigquery` và sentinel tại
`loading_method.credential.hmac_key_secret`; kết quả:

```text
secret_in_plain_config=True
secret_in_encrypted_payload=False
```

`actors.create()` sau đó lưu `config` vào `configuration_json` và audit tại
`backend/app/services/actors.py:317-384`. Vì actor detail trả configuration, lỗi
này có thể biến một secret thành dữ liệu đọc được bởi role chỉ có VIEW.
`destination-bigquery` lại đang được công bố `SUPPORTED`; chứng nhận hiện mới
dùng `loading_method = Standard`, nên không chạm nhánh HMAC bị lỗi.

**Acceptance bắt buộc:**

1. Server là authority và walk object/array/`oneOf` ở mọi độ sâu, giữ nguyên
   shape để merge lại trước engine call; không chỉ sửa `splitSecrets()` ở FE.
2. Sinh sentinel cho mọi path có `airbyte_secret`, `writeOnly`, `secret` hoặc
   `format=password` trong từng connector `SUPPORTED`; sentinel không được có
   trong Product DB config, audit, log hoặc API response, nhưng phải round-trip
   đúng đến adapter check.
3. Scan dữ liệu hiện hữu và audit cho key nhạy cảm; có migration/remediation để
   mã hóa lại hoặc buộc nhập lại credential.
4. Trước khi fix hoàn tất, khóa nhánh BigQuery HMAC hoặc hạ connector khỏi
   production launch scope; không để UI cho chọn một mode chưa an toàn.

### P0-CORE-CONSISTENCY - chưa có durable saga giữa DB và engine

Source/destination create gọi engine tại `backend/app/services/actors.py:341-379`;
pipeline create gọi engine tại `backend/app/services/pipelines.py:316-380`. API
route chỉ commit sau khi service trả về. Nếu engine success nhưng flush/audit/
commit DB fail, không có durable record để retry hoặc xóa engine resource. Các
comment compensation hiện chỉ xử lý engine call tự fail, không xử lý commit
fail sau success.

Reconcile có thể báo một số resource foreign/missing, nhưng không phải operation
ledger và không chứng minh resource chứa customer credential sẽ được dọn đúng,
idempotent và có cảnh báo.

**Acceptance bắt buộc:** operation/outbox có các trạng thái tối thiểu
`PENDING -> ENGINE_CREATED -> COMMITTED` và `COMPENSATION_REQUIRED`; external
idempotency key gắn product resource id; worker retry/compensate; alert khi quá
SLO. Fault-injection tests phải fail DB tại từng điểm sau engine create/update,
restart process, rồi chứng minh không còn orphan và không tạo duplicate.

### P0-REL-SEC - script Application có thể in token/secret vào CI log

`scripts/airbyte-application.py:106-129` và `132-152` lưu `body[:120]` của mọi
login/create attempt rồi `main()` in toàn bộ attempt. HTTP 200 có thể chứa
`access_token`, `clientSecret` hoặc `client_secret`, trái với comment "prints
nothing secret". Lane K8s gọi script này tại `.github/workflows/ci.yml:353-373`.

Ngoài ra sign-in/create endpoint trong script được ghi rõ là chưa chạy trực tiếp
trên Airbyte 1.8.5. Test hiện chỉ mock response; CI remote chưa chạy. Vì vậy lane
không được coi là đã sửa cho tới khi có run thật xanh.

**Acceptance bắt buộc:** không log raw body; parse và redact theo key ở mọi
status; test với token/secret nằm đầu, giữa và cuối body. Production tiếp tục
dùng Application do operator tạo và secret manager giữ. CI hoặc nhận một
credential test đã provision an toàn, hoặc dùng một bootstrap flow đã chứng
minh trực tiếp; không biến endpoint đoán thành release evidence.

### P1-UX-01 - primary wizard đang là full registry browser

Live API đo được:

| Wizard | API rows | Payload | Selectable | Bị khóa trong 24 card đầu |
|---|---:|---:|---:|---:|
| Source | 598 | ~515 KB | 5 | 20 |
| Destination | 56 | ~56 KB | 2 | 22 |

`ActorWizard` luôn gọi toàn bộ type tại
`frontend/src/components/integrations/ActorWizard.tsx:55-58` và
`ConnectorPicker` lọc/render cả connector không selectable tại
`frontend/src/components/integrations/ConnectorPicker.tsx:44-57,98-125`.

**Product decision:** create wizard chỉ lấy `selectable=true`, có pagination và
đưa 2-6 launch connector lên ngay. Full 654 connector chỉ tồn tại ở màn
Connectors/Admin, nơi trạng thái beta/blocked là thông tin hữu ích. Mục tiêu
payload first step dưới 50 KB và không có card disabled trong primary path.

### P1-UX-02 - không có một luồng setup liên tục

Save source/destination luôn đẩy tới detail tại
`frontend/src/components/integrations/ActorWizard.tsx:118-122`; detail chỉ có
test/discover/enable/delete, không có "tiếp tục tạo destination/pipeline" tại
`frontend/src/components/integrations/ActorDetailPage.tsx:127-188`. Pipeline
wizard tự tải lại danh sách và không nhận source/destination từ URL tại
`frontend/src/app/(main)/pipelines/new/page.tsx:45-74`.

Acceptance cho pilot:

1. First-run journey duy nhất: chọn source -> cấu hình/test -> chọn destination
   -> cấu hình/test -> chọn streams -> schedule -> run đầu tiên.
2. Context được prefill, tên có default hợp lý, advanced option thu gọn; Back
   không mất dữ liệu.
3. Mỗi connector launch có link setup guide và chỉ dẫn credential cụ thể.
4. Sidebar first-run chỉ nổi Setup và Activity; catalog, Builder, audit, alert,
   engine settings nằm dưới Advanced/Admin.
5. Browser UAT desktop + mobile cho production bootstrap, happy path, bad
   credential, engine down, retry/cancel và analyst read-only. Các audit script
   hiện có là nền tốt nhưng phải chạy trên commit release và publish artifact.

### P1-PERF/SEC - phải đóng trước load/soak

- Actor list query connector + owner theo từng row tại
  `backend/app/api/v1/actors.py:43-53`; pipeline list gọi source/destination,
  connector và owner theo từng row tại `backend/app/api/v1/pipelines.py:39-57`;
  run list cũng load user từng row tại `backend/app/api/v1/runs.py:74-83`.
  Batch-load/map trước khi test 200-row pages.
- Login lock một account sau 5 lỗi và trả `429` riêng cho user tồn tại tại
  `backend/app/api/v1/auth.py:48-65`; hiện có thể bị dùng để enumerate/DoS một
  email admin. Kết hợp per-IP/per-account throttling, generic response và admin
  recovery thay vì fixed lockout đơn lẻ.

### Release sequence đóng

1. Đóng P0-AUTH và P0-SECRET; chạy browser bootstrap và sentinel suite.
2. Đóng durable saga/outbox; chạy fault injection và engine reconciliation.
3. Redact/bootstrap Application và có CI auth-enabled thật từ clean remote.
4. Làm guided FE journey, rồi owner tự hoàn thành UAT từ account mới mà không
   cần dev hướng dẫn.
5. Sau đó mới quay lại exact-topology golden path, true cancel, paired restore,
   legal/on-call/evidence v2 và clean runner.

Working tree đang có nhiều file modified/untracked của dev. Không tag/push
release trước khi các thay đổi được review thành một commit sạch; kết quả xanh
trên laptop không bind được vào một artifact khi tree còn dirty.

## PM review v15 - teardown/clean-room gate (2026-08-25)

Review này kiểm trực tiếp commit `f6ce6ec`, code installer, CI, release gate,
Docker labels/volumes, Kubernetes workloads và local registry. Kết luận không
dựa trên comment trong tài liệu.

### Quyết định

1. Compose demo `appbi-pipeline` đã được reset; dry-run và kiểm tra sau cùng
   không có tài nguyên `appbi-ai` trong phạm vi xóa.
2. Owner quyết định xóa cả RC1 dù chưa có paired backup/release artifact. Đã xóa
   `appbi-rc1-control-plane`, `rc1-*`, volumes, network, kube context và local
   registry. Đây là clean reset có chủ đích, không phải DR drill.
3. Không release production. Namespace `appbi` trong cluster chưa có workload,
   và cluster hiện đã bị xóa, nên chưa hề có exact-topology golden path.

### Kết quả clean reset thực tế

- `.env` cũ được chuyển ra `D:\Appv2\_appbi-pipeline-local-backup`; installer
  chạy khi repository không có `.env` và tự sinh hai secret mới.
- Cold project build thành công; Postgres volume và schema được tạo từ rỗng,
  Alembic ở head `f2c0a15b8e37`.
- Docker Desktop chỉ còn project `appbi-ai` và `appbi-pipeline`; AppBI có API,
  worker, frontend, proxy, Postgres Running và migrate `Exited (0)`.
- UI, shallow readiness và deep readiness đều HTTP `200`.
- Fresh E2E pass source/destination check, discover, pipeline creation và full
  sync `2.500` records; incremental đọc 2 record mới do test chèn.
- Cancel vẫn inconclusive vì run hoàn thành trước khi cancel có hiệu lực;
  Connector Builder chưa được chạy trên database mới.

Kết quả này đóng câu hỏi "máy sạch có dựng được demo không": **có**. Nó không
đóng câu hỏi production vì demo dùng `AIRBYTE_EMBEDDED`, local Postgres, không
TLS và không có durable registry/backup.

### Findings theo mức độ

**P0 - Production install vẫn tự chặn trước rollout.**

`install_k8s()` gọi `verify_engine()` tại `scripts/production.py:962`.
`verify_engine()` lấy credential qua `resolve_secret()`, nhưng `secret://` trả
chuỗi rỗng theo thiết kế (`scripts/production.py:1179-1190`). Probe vì thế gửi
không credential tới auth-enabled Airbyte và biến `401` thành fatal tại
`scripts/production.py:1114-1119`. Post-deploy check trong Pod là đúng, nhưng
không bao giờ tới được nếu pre-deploy check này dừng trước.

Acceptance: preflight chỉ kiểm network/TLS hoặc chạy một authenticated probe có
nguồn credential hợp lệ; engine identity/authenticated contract phải được kiểm
từ AppBI Pod sau rollout. Có test hành vi với `secret://` + response `401`, không
chỉ search source text.

**P0 - Không có release có thể phục hồi độc lập.**

Không có Git remote, `deploy/production.yaml`, `certification*.json`,
`mirror-lock.json`, DB dump, SBOM/provenance. Local registry chứa 4 image trong
anonymous volume. Evidence hiện tại là schema v1. Xóa RC1 bây giờ làm mất input
duy nhất của clean-room thay vì chứng minh clean-room.

Acceptance: source/tag ở remote; image/chart/connectors ở durable registry bằng
digest; evidence v2 + certification + mirror lock + paired backup ở artifact
store ngoài máy; release gate pass trên commit đang deploy.

**P0 - Exact production topology chưa chạy.**

Airbyte chạy trong namespace `airbyte`, nhưng namespace `appbi` chỉ có hai
Secret. Không có migrate Job, API, worker, frontend hoặc ingress. Vì vậy G1 của
engine không đồng nghĩa sản phẩm đã chạy production topology.

Acceptance: một clean install tạo đủ AppBI workloads, deep readiness `200`, rồi
golden path và recovery/restart/cancel chạy trên đúng deployment đó.

**P0 - CI K8s lane chưa tái tạo được RC1.**

`.github/workflows/ci.yml:299-370` cài values file vẫn chứa `REPLACE_ME` và
`EXTERNAL_POSTGRES_HOST`, không provision external DB/secret tương ứng, rồi gọi
Airbyte APIs không credential dù auth bật. Repository không có remote nên chưa
có run độc lập nào bác bỏ quan sát này.

Acceptance: lane provision toàn bộ dependency hoặc nhận secret/DB endpoint từ
CI environment, tạo Application credential theo runbook, chạy authenticated
golden path và upload artifact. Không có placeholder trong rendered Helm values.

**P0 - Release gate đang block đúng.**

`compatibility.yaml` còn `LIC-001: NOT_CLEARED` và
`UAT-001..015: NOT_PROVEN`. Không đổi trạng thái bằng comment hoặc unit test;
chỉ legal approval và per-scenario run evidence trên target topology được phép
đóng hai gate này.

**P1 - `doctor` vẫn có kết quả âm giả.**

`scripts/production.py:1510-1513` kiểm placeholder trên source overlay, trong
khi source overlay chủ ý giữ placeholder fail-closed và installer mới chuyển
logic sang rendered output. Một install đúng vẫn có thể bị `doctor` báo không
production-ready.

Acceptance: `doctor` render cùng config và kiểm đúng artifact sẽ apply; test gọi
doctor path thực, không assert chuỗi.

**P1 - Chưa có teardown orchestration có guardrail.**

`production.py` chỉ có install/upgrade/status/doctor/logs/rollback, không có
`rehearse`, `uninstall` hay `clean-room`. RC1 containers dùng tên tự do và
anonymous volumes. Người vận hành hiện phải ghép lệnh tay, dễ xóa nhầm project.

Acceptance: một command clean-room có dry-run mặc định, exact allowlist cho
project/cluster/container/volume, từ chối mọi resource mang label
`com.docker.compose.project=appbi-ai`, bắt buộc backup/export trước teardown và
in manifest những gì đã xóa.

### Final production exit criteria

PM chỉ review release lần cuối khi cùng một commit/digest có đủ: remote source;
durable mirror; filled production config; clean runner install; `doctor` exit 0;
exact-topology golden path gồm cancel/restart; paired DR restore; evidence v2 và
certification; legal/on-call/alert/rollback evidence. Chi tiết thao tác ngắn gọn
ở đầu `CURRENT_STATUS.md` mục PM v15.
...
## Dev — PM v15 response (2026-08-25)

Bốn finding PM giao cho dev: hai P0 (preflight, CI lane) và hai P1 (`doctor`,
teardown). Cả bốn đã sửa. Các P0 còn lại — legal, remote, durable registry,
exact topology, paired DR — cần người hoặc hạ tầng ngoài máy này, không đụng
tới.

### P0 — preflight tự chặn trước rollout

PM đúng, và lỗi nghiêm trọng hơn mô tả: **không một production install nào có
thể qua được bước này.** `verify_engine()` lấy credential qua `resolve_secret()`,
`secret://` trả chuỗi rỗng theo thiết kế, nên probe gửi request không credential
tới engine bật auth, nhận `401`, và coi đó là fatal. Post-deploy check trong Pod
viết đúng nhưng không bao giờ chạy tới.

Preflight giờ chỉ quyết định điều nó thật sự biết được: **reachability**. DNS
phân giải, port trả lời, TLS terminate — `401` chứng minh cả ba, nên là pass.

| Response | Kết luận |
|---|---|
| `0` (không ai trả lời) | fatal — sai URL/DNS/network |
| `401`/`403`, không có credential đọc được | **pass** — engine sống và đang enforce auth |
| `401`/`403`, có credential đọc được (`env://`) | fatal — credential trong config sai |
| `5xx` | fatal |
| `200` + version khác pin | fatal |

Identity, version và authenticated contract kiểm từ AppBI Pod sau rollout bằng
`verify_engine_in_pod()`, tức từ chính process giữ credential và đi đúng đường
mạng runtime dùng.

Test hành vi theo đúng acceptance PM yêu cầu:
`backend/tests/test_installer_preflight.py` dựng config với `secret://` refs,
stub tầng HTTP trả `401`, và assert `verify_engine()` **không** raise. Tám case,
gồm cả chiều ngược lại (credential đọc được mà bị từ chối thì vẫn fatal) và
`resolve_secret("secret://...") == ""`.

### P1 — `doctor` báo âm giả

Cùng một lỗi installer vừa bỏ, còn sót lại ở `doctor`. Đã cho `doctor` render
đúng config rồi kiểm artifact sẽ apply, **dùng chung `static_gates()` với
`install`** — để một lần sửa không thể chỉ vào một lệnh.

Không assert chuỗi. `backend/tests/test_production_render.py` gọi `cmd_doctor()`
thật (chỉ stub network) từ repository root. **Đã đo bằng cách khôi phục code cũ:**
config đúng bị báo `'appbi.example.internal' is still in the rendered manifests`
trong khi config đặt `appbi.acme.io` — đúng kết quả âm giả PM mô tả. Ba test,
một trong đó assert `doctor` và `install` cho cùng kết quả trên cùng config.

Chi tiết đáng ghi: chạy pytest từ `backend/` thì code cũ **pass**, vì overlay
path là root-relative nên không resolve được và gate im lặng. Test phải chdir về
root mới tái hiện được lỗi.

### P1 — teardown chưa có guardrail

Thêm `python scripts/production.py clean-room`. Ba tính chất, mỗi cái ứng với
một thứ đã hoặc suýt hỏng:

| | |
|---|---|
| **Dry-run mặc định** | `--apply` mới xóa. Không có `--yes`, không có `--force` |
| **Allowlist theo label, không theo tên** | Compose project label của Docker quyết định; `com.docker.compose.project=appbi-ai` bị từ chối và **ghi vào manifest** |
| **Backup bắt buộc** | Backup fail ⇒ dừng, chưa xóa gì. Muốn bỏ phải `--accept-data-loss` |

Chạy thật trên máy này (dry-run, không xóa gì):

```text
compose project: appbi-pipeline
containers   6      volumes 3      networks 2
protected    10 resource của appbi-ai bị từ chối
```

Trong 10 resource được bảo vệ có network tên **`appbi-net`** — không mang tiền tố
`appbi-ai`, thuộc về `appbi-ai`. Còn `appbi-pipeline_appbi` thì ngược lại. Mọi
rule viết theo tên (`appbi`, `appbi-*`, `*pipeline*`) sai ít nhất một trong hai;
label đúng cả hai. Đó là lý do không dùng name matching.

### P0 — CI K8s lane

Ba lỗi PM nêu là cùng một lỗi: lane report về một deployment không thể là cái
production sẽ chạy.

**1. Placeholder.** Không xóa placeholder khỏi profile — chúng ở đó để một
`helm install` chưa sửa fail closed. Thay vào đó thêm
`scripts/render-engine-values.py`: substitute rồi **từ chối ghi file nếu còn
placeholder**. Phát hiện theo *shape* (`SCREAMING_SNAKE_CASE`, `<brackets>`,
example host) chứ không theo danh sách token cố định — danh sách sẽ cũ đi ngay
lần đầu ai đó thêm placeholder mới, mà đó chính là loại lỗi này. Nó bắt được cả
`global.airbyteUrl: http://airbyte.rc1.internal` — trỏ vào host RC1 đã xóa, PM
chưa liệt kê. Secret truyền qua `--set-from-env` nên không xuất hiện trên command
line hay trong log.

**2. Dependency.** `postgresql.enabled: false` mà không ai tạo database ngoài là
ngõ cụt. Thêm `deploy/kubernetes/airbyte/external-postgres-ci.yaml`: Postgres
riêng trong namespace `airbyte-db`, password sinh lúc chạy vào Secret
`airbyte-config-secrets`, không có credential nào trong repository.

**3. Unauthenticated calls.** Thêm `scripts/airbyte-application.py` tạo
Application trong run, và `verify-engine-api.py` giờ đổi client credentials lấy
bearer token (trước chỉ nói Basic, thứ 1.x không nhận). Credential forward vào
in-network probe bằng `-e NAME` — kế thừa chứ không ghi giá trị vào command line.
Lane thêm một step assert `engine_type == "AIRBYTE_API"` và engine dependency
healthy, vì deep-readiness 200 với engine tắt auth trông y hệt. Artifact upload
**không** gồm rendered values hay `application.json`.

**Điều cần nói rõ:** trong `airbyte-application.py`, chỉ
`instance_configuration/setup` và `applications/token` là đã chạy thật trên
Airbyte 1.8.5. Sign-in và create-application làm qua UI trong rehearsal nên
request shape **không được ghi lại**. Script thử các candidate đã ghi trong
runbook, nhưng **định nghĩa thành công là token exchange chạy được**, không phải
một endpoint trả 200 — nên nó hoặc trả credential dùng được thật, hoặc fail và in
ra từng lần thử kèm response. Không có đường nào rơi về unauthenticated.

Lane này **chưa được chứng minh xanh**: chưa có remote và chưa có runner nào chạy
nó. Đó là R1, không phải thứ dev đóng được ở đây.

### Redis — dọn nốt phần còn sót

Vòng v14 bỏ Redis khỏi Compose/settings/requirements nhưng bỏ sót
`scripts/stack.py`, nơi `lite` profile vẫn liệt kê `redis` là service phải khởi
động — tức **`stack.py lite` đã hỏng**, không phải chỉ là comment cũ. Đã sửa,
cùng warning của `doctor` và bốn comment còn nhắc Redis.

### Verification

```text
backend pytest                287 passed, 18 skipped   (v14: 255)
docker compose up -d          6 container, UI 307, readyz 200, deep 200
production.py status          exit 0
production.py doctor (demo)   exit 1 — LIC-001, UAT, on-call. Không còn placeholder giả
clean-room (dry-run, thật)    11 resource in scope, 10 resource appbi-ai bị từ chối, không xóa gì
CI workflow                   42/42 run block hợp lệ cú pháp bash; mọi step có run hoặc uses
kustomize x3                  render sạch
compose config x2             hợp lệ
```

### Vẫn mở, và tại sao

`LIC-001` (legal), `UAT-001..015` (per-scenario evidence trên target topology),
R1 remote, R2 durable registry/mirror-lock, R3 clean-runner install, R4
exact-topology golden path, R5 paired DR restore, R6 evidence v2 +
certification. Không cái nào đóng được bằng code trên laptop này.
...
Viet day du trong [docs/ADR-001-database-topology.md](ADR-001-database-topology.md).
