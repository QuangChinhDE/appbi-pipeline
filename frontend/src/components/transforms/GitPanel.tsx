...
export function GitPanel({
  ...
  if (loading && !status) {
    return <p className="p-3 text-caption text-text-tertiary">{t('tf.git.loading')}</p>;
  }
  if (!status) {
    return (
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center gap-1.5 border-b border-[rgb(var(--border-line))] px-2 py-1.5">
        ...
        {status.behind && (
          <Badge variant="warning" size="xs">{t('tf.git.behind')}</Badge>
        )}
        <Button
          variant="ghost" size="xs" onClick={onRefresh} loading={busy}
          aria-label={t('tf.git.refresh')} className="ml-auto"
        >
        ...
        {status.changes.length === 0 ? (
          ...
          <>
            <p className="px-2 py-1.5 text-tiny uppercase tracking-wide text-text-quaternary">
              {t('tf.git.changes', { n: status.changes.length })}
            </p>
            {status.changes.map((change) => (
              ...
              >
                <input
                  ...
                  aria-label={t('tf.git.selectFile', { path: change.path })}
                />
                <span className={cn(
      ...
      {canEdit && status.changes.length > 0 && (
        <div className="border-t border-[rgb(var(--border-line))] p-2">
          <textarea
            ...
            rows={2}
            placeholder={t('tf.git.commitMessage')}
            className={cn(
          ...
          <p className="mt-1 text-tiny text-text-quaternary">
            {t('tf.git.selectedCount', {
              n: stagedPaths.length, total: status.changes.length,
            })}
          </p>
        </div>
      )}
      ...
      <Modal
        ...
        title={t('tf.git.switchBranch')}
      >
        <div className="space-y-2">
          {status.changes.length > 0 && (
            <p className="rounded-md bg-warning/10 p-2 text-caption text-warning">
              {t('tf.git.switchWarning', { n: status.changes.length })}
            </p>
          )}
            ...
            {branches.map((branch) => (
              <li key={branch.name}>
                ...
                >
                  ...
                  {branch.protected && (
                    <Badge variant="subtle" size="xs" className="ml-auto">{t('tf.git.protected')}</Badge>
                  )}
                  {branch.current && (
