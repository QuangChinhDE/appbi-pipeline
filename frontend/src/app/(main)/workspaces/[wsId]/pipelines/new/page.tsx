...
export default function NewPipelinePage() {
  ...
  const create = useMutation({
    ...
    onSuccess: (pipeline) => {
      ...
      router.push(ws(`/pipelines/${pipeline.id}`));
    },
        ...
        <EmptyState
          ...
          action={
            <div className="flex gap-2">
              <Link href={ws('/sources/new')}><Button variant="primary">{t('sources.add')}</Button></Link>
              <Link href={ws('/destinations/new')}><Button variant="secondary">{t('destinations.add')}</Button></Link>
            </div>
          }
        />
      ) : (
