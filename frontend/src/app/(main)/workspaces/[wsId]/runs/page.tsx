                ...
                {items.map((run) => (
                  <tr key={run.id} className="transition-colors hover:bg-surface-2/60">
                    <td className="px-4 py-2.5">
                      <Link href={ws(`/runs/${run.id}`)} className="inline-block py-1 font-mono text-caption text-brand hover:underline">
                        {run.short_id}
                      </Link>
                    ...
                    <td className="px-3 py-2.5">
                      {run.run_type === 'TRANSFORM' && run.transform ? (
                        <Link href={ws(`/transforms/${run.transform.id}`)} className="inline-block py-1 text-caption text-text-primary hover:text-brand">
                          {run.transform.name}
                        </Link>
                      ) : run.pipeline ? (
                        <Link href={ws(`/pipelines/${run.pipeline.id}`)} className="inline-block py-1 text-caption text-text-primary hover:text-brand">
                          {run.pipeline.name}
                        </Link>
                      ) : '-'}
                    </td>
