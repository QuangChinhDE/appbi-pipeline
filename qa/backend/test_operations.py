...
def test_every_ops_script_forces_utf8_output() -> None:
    ...
    # Paths, because the suite moved out of `scripts/` into `qa/` and this list
    # did not follow it -- `scripts/e2e.py` and `scripts/verify-egress.py` have
    # not existed for a while.
    scripts = [
        "scripts/stack.py",
        "scripts/backup.py",
        "scripts/rotate-kek.py",
        "scripts/release-gate.py",
        "scripts/airbyte-workspace.py",
        "qa/probes/verify-egress.py",
        "qa/e2e/e2e.py",
    ]
    for name in scripts:
        path = ROOT / name
        assert path.exists(), f"{name} is in this list but not on disk"
        assert "force_utf8" in path.read_text(encoding="utf-8"), (
            f"{name} will die on non-ASCII output")
...
# ── the stack has to come up on an empty machine ─────────────────────────────
# Every one of these was invisible until the volumes were wiped, because each
# only bites while Postgres is still running its init scripts -- which happens
# exactly once per data directory. A stack that has been restarted a hundred
# times can still be unable to start from zero.

@repo_only
def test_the_postgres_healthcheck_probes_over_tcp() -> None:
    """`pg_isready` without a host talks over the unix socket, and the official
    image keeps a temporary server on that socket while it runs
    /docker-entrypoint-initdb.d. The probe therefore reports healthy in the
    middle of initialisation, `service_healthy` fires, and everything that
    depends on the database starts against one answering "the database system
    is starting up".

    Temporal is the one that dies: its schema setup fails, and the Airbyte
    server then exits 1 on the version check with an error naming neither
    Postgres nor initialisation.
    """
    import yaml
    ...
    assert "-h 127.0.0.1" in test, (
        f"the healthcheck probes the unix socket, so it goes green during "
        f"initdb: {test}")


@repo_only
def test_the_engine_waits_for_temporal_rather_than_for_luck() -> None:
    """The Airbyte server opens a Temporal client while wiring its beans and
    exits 1 if it cannot reach one.

    It carried no dependency on Temporal at all and worked anyway, because the
    storage-init -> minio chain delayed it long enough. Removing MinIO removed
    the delay and the next cold start failed with
    `UnknownHostException: airbyte-temporal` -- which reads like a network
    fault rather than a missing edge in the graph.

    `service_started` is not enough either: Temporal listens on 7233 before it
    has created its databases and schema, so readiness has to be its own
    health check.
    """
    import yaml
    ...
    assert "healthcheck" in services["airbyte-temporal"], (
        "Temporal has no health check, so dependants can only wait for its "
        "port to open")
    probe = " ".join(services["airbyte-temporal"]["healthcheck"]["test"])
    assert "127.0.0.1" not in probe, (
        "the entrypoint binds Temporal to the container IP, so a loopback "
        "probe is refused by a healthy server")

    for name in ("airbyte-server", "airbyte-worker"):
        depends = services[name]["depends_on"]
        assert "airbyte-temporal" in depends, f"{name} does not wait for Temporal"
